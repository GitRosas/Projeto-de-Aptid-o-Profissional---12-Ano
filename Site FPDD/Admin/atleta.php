...
?>
    ...
    <body>
        ...
        <br>
        <center><h1><b>Atletas</b></h1></center>
        <br>
        <br>
        ...
        $stmt = $pdo->query("select * from atletas_inscritos, escolas where escolas.Cod_Escola = atletas_inscritos.Cod_Escola;");
        $stmt2 = $pdo->query("select count(*) as num from atletas_inscritos;");
        ...
        if ($fetch = $stmt2->fetch()) {
            ...
            if ($num_rows > 0) {
                ...
                foreach ($stmt as $row) {
                    echo '<tr>';
                    echo '<th scope="row">' . $row['Dorsal'] . '</th>';
                    echo '<td>' . $row['Nome_atleta'] . '</td>';
                    echo '<td>' . $row['Nome_escola'] . '</td>';
                    echo "<td> <a href='./eliminarAtleta.php?Dorsal=$row[Dorsal]'>Eliminar</a> </td>";
                    echo "<td> <a href='./alterarAtleta.php?Dorsal=$row[Dorsal]&Nome_atleta=$row[Nome_atleta]&Cod_Escola=$row[Cod_Escola]'>Alterar</a> </td>";
                }
                echo '</tbody>';
                echo '</table>';
                echo '</div>';
